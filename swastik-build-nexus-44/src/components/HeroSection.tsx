...
const HeroSection = () => {
  ...
  return (
    <>
      <section id="home" className="relative w-full bg-black flex flex-col justify-center">
        ...
        {/* Ribbon at the bottom - Desktop - Placed outside the relative wrapper so it sits below! */}
        <div
          className="w-full relative z-20 hidden md:flex flex-row items-center justify-center gap-8 px-4 py-2"
          style={{ backgroundColor: '#1953B4' }}
        >
          {['Location', 'Property Type', 'Configuration'].map((label) => (
            <button key={label} className="flex items-center gap-2 text-white font-medium px-4 py-1 focus:outline-none w-auto justify-center hover:bg-white/10 rounded-lg transition-all duration-300">
              {label} <span className="ml-1">&#9662;</span>
            </button>
          ))}
          <button className="flex items-center justify-center text-white px-4 py-1 focus:outline-none w-auto hover:bg-white/10 rounded-lg transition-all duration-300">
